...
func (_q *RolePermissionQuery) QueryCollection() *GroupQuery {
...
func (_q *RolePermissionQuery) WithCollection(opts ...func(*GroupQuery)) *RolePermissionQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RolePermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RolePermission, error) {
	...
	if query := _q.withCollection; query != nil {
		if err := _q.loadCollection(ctx, query, nodes, nil,
			func(n *RolePermission, e *Group) { n.Edges.Collection = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RolePermissionQuery) loadCollection(ctx context.Context, query *GroupQuery, nodes []*RolePermission, init func(*RolePermission), assign func(*RolePermission, *Group)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*RolePermission)
	for i := range nodes {
...
func (_q *RolePermissionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCollection != nil {
			_spec.Node.AddColumnOnce(rolepermission.FieldCollectionID)
		}
	}
